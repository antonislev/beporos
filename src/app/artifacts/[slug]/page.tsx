...
export default function ArtifactPage() {
  ...
  return (
    <section className="px-6 md:px-10 py-24 max-w-[1000px] mx-auto">
      <Link href="/artifacts" className="font-display text-[11px] tracking-[2px] text-[var(--color-muted)] hover:text-[var(--color-pink)] transition-colors mb-10 inline-block">
        ← BACK
      </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mt-6">
        ...
        <div className="flex flex-col">
          <h1 className="font-display text-[clamp(22px,3vw,30px)] font-bold tracking-[2px] uppercase mb-4">
            {product.name}
          </h1>

          <div className="w-full h-[1px] bg-[var(--color-border)] mb-5" />

          <p className="text-[var(--color-muted)] leading-[1.8] mb-8">
            {product.description}
          </p>
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2">
              {product.sizes.map((size) => (
                <span key={size} className="font-display text-[11px] px-4 py-2 border border-[var(--color-border)] text-[var(--color-muted)]">
                  {size}
                </span>
              ))}
            </div>
          ...
          <div className="mt-auto space-y-3">
            <a href="https://instagram.com/beporos" target="_blank" rel="noopener noreferrer" className="block font-display text-[11px] tracking-[2px] uppercase py-4 bg-[var(--color-pink)] text-[var(--color-text-inv)] hover:bg-[var(--color-pink-hover)] transition-colors text-center">
              DM @BEPOROS TO GRAB ONE
            </a>
            <p className="font-display text-[10px] tracking-[1px] text-[var(--color-muted)] text-center">
              100 units. No restock. Find us on the island this summer.
            </p>
          </div>
        </div>
      </div>
