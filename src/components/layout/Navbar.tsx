  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-10 py-4 bg-[var(--color-bg)]/95 backdrop-blur-sm border-b border-[var(--color-border)]">
        <Link href="/" className="relative w-[100px] h-[34px] md:w-[120px] md:h-[40px]">
          <Image src="/images/brand/logo.png" alt="beporos" fill className="object-contain object-left" sizes="120px" priority />
        </Link>

        <div className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            <Link key={link.href} href={link.href} className="font-display text-[11px] tracking-[2px] uppercase text-[var(--color-muted)] hover:text-[var(--color-pink)] transition-colors">
              {link.label}
            </Link>
          ))}
